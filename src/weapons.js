...
//  Wider, softer wedge echo — same filled-light concept but dimmer and bluer.
const _afterFrag = /* glsl */`
  ...
  void main(){
    ...
    float glow  = exp(-cy * cy * 22.0);          // wide, soft — 3× wider than main blade
    float outer = smoothstep(0.45, 1.0, vUv.y) * 0.5;  // soft outer brightening
    float taper = 1.0 - smoothstep(0.80, 1.0, vUv.x) * 0.55;
    float base  = smoothstep(0.0, 0.06, vUv.x);

    vec3  col   = mix(uColor * 0.8, vec3(0.6, 0.82, 1.0), outer);
    float alpha = (glow * 0.50 + outer * 0.25) * taper * base * uFade;
    gl_FragColor = vec4(col, clamp(alpha, 0.0, 1.0));
  }
`;
