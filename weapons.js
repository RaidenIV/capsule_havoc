...
function getOrbitRingDefsFromTier(tier) {
  ...
  const speedBase = (1.7 * 4.0) + Math.max(0, state.upg?.orbitSpeed || 0) * 0.20;
...
const _targetedLaserGlowMat = new THREE.MeshStandardMaterial({
  color: 0x3b82ff,
  emissive: 0x3b82ff,
...
function _makeTargetedLaserVisual() {
  const g = new THREE.Group();
...
function _acquireTargetedLaserVisual() {
  const g = _targetedBulletPool.pop() || _makeTargetedLaserVisual();
  g.visible = true;
  g.traverse(obj => { obj.visible = true; });
  return g;
}
function _releaseTargetedLaserVisual(g) {
  if (!g) return;
  scene.remove(g);
  g.visible = false;
  g.traverse(obj => { obj.visible = false; });
...
function _makeTargetedShotVisual() {
  return _acquireTargetedLaserVisual();
}
...
function _updateTargetedShots(worldDelta) {
  ...
  for (let i = state.targetedShots.length - 1; i >= 0; i--) {
    ...
    if (b.life <= 0) { _releaseTargetedLaserVisual(b.obj); state.targetedShots.splice(i, 1); continue; }
    let hit = false;
    ...
    for (let n = candidates.length - 1; n >= 0; n--) {
      ...
      if (dx*dx + dz*dz < 0.78 * 0.78) {
        ...
        _releaseTargetedLaserVisual(b.obj);
