...
export function getWeaponConfig() {
  ...
  const orbitSpeed = (1.7 * 4.0) + Math.max(0, state.upg?.orbitSpeed || 0) * 0.20;
...
export function updateXP(amount) {
  ...
  while (state.playerLevel < 100) {
    ...
    // Queue one shop for every level gained, including boss levels.
    state.pendingShop = Math.max(0, Number(state.pendingShop) || 0) + 1;
  }

  if (state.playerLevel >= 100 && !state.gameOver) {
    state.playerXP = 0;
  }
  syncXPUI();

  if (state.playerLevel >= 100 && !state.gameOver) {
    Promise.resolve().then(() => import('./gameFlow.js'))
      .then(m => m.triggerVictory?.())
      .catch(() => {});
  }
}
